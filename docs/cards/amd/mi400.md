...
| 型号 | 定位 | 显存 | 算力 (FP4) |
|------|------|------|-----------|
| **MI455X** | 旗舰训练+推理（Helios 专用） | 432GB HBM4 | 40 PFLOPS (dense) |
| **MI450X** | 大规模训练+推理 | 432GB HBM4 | 40 PFLOPS (dense) |
| **MI430X** | HPC / 主权 AI（FP64 加速） | 432GB HBM4 | 40 PFLOPS (dense) |

> 📌 **三款 SKU 均配备 432GB HBM4 + 19.6 TB/s 带宽**，共享 CDNA 5 计算核心。MI455X 专为 Helios 机架优化；MI450X 面向独立大规模部署；MI430X 额外支持硬件级 FP64 与 CPU+GPU 混合计算。
