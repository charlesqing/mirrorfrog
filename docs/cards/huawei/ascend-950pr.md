...
| **MXFP4** | **1,784 / 1,561 TFLOPS**（128GB / 112GB 配置） |
| **FP8 / HiF8 / MXFP8** | **919 / 804 TFLOPS**（官方宣传口径取整 1 PFLOPS） |
| **INT8** | **919 / 804 TOPS** |
| **BF16 / FP16** | **486 / 425 TFLOPS** |
| **TF32** | **243 / 212 TFLOPS** |
...
> 📌 **重要订正 · FP16 反而低于 910C（2026-08 联网核实）**：本站此前记载 950PR 的 FP16 为「推测 ~780 TFLOPS」，**该推测错误**。据《昇腾 950 NPU 架构白皮书》明确值：950PR 的 **BF16/FP16 仅 486 TFLOPS**，明显低于上一代 910C 的 ~800 TFLOPS。
>
> 原因并非退步，而是**架构取舍**：950 系列定位「大模型推理 Prefill + 推荐业务」，把 die 面积与晶体管预算倾斜给 **FP8/FP4 低精度单元**（FP8 919 vs FP16 486，比例接近 2:1），并首次引入自研 HBM。因此：
> - **低精度推理（FP8/FP4）**：950PR 大幅领先 910C，FP4 达 1.78 PFLOPS（910C 不支持 FP4）
> - **FP16 训练**：910C 仍以 800 vs 486 领先，**做 FP16 密集训练不应选 950PR**
>
> 选型时应按**目标精度**而非「代数」判断：950PR 是推理/预填充卡，不是 910C 的全面升级。
