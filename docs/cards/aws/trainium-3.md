...
| FP8 算力（密集） | 1,299 TFLOPS | **2,520 TFLOPS（2.52 PFLOPS）** | **~1.94×** |
| BF16/FP16 | 667 TFLOPS | 1,260 TFLOPS（推测，FP8 一半） | ~1.89× |
| **每芯片 FP8 算力提升** | — | **~1.94×** | 按官方单芯片 2.52 PFLOPS 计算 |
| **能效（每瓦性能）** | — | **4×** | 官方数据（机柜级） |
| **内存带宽提升** | — | **3.9×** | 官方数据（机柜级） |
| 发布时间 | 2024-12 | **2025-12** | — |

> ⚠️ **口径澄清（依据 aws.amazon.com 官方页面）**：官方 **4.4× 是机柜级**对比——Trn3 UltraServer（144 芯片，**362 MXFP8 PFLOPS**）vs Trn2 UltraServer（64 芯片，83.2 PFLOPS），362 ÷ 83.2 ≈ **4.35×**。**单芯片**口径则为 Trainium 3 **2.52 PFLOPS** vs Trainium 2 1,299 TFLOPS，约 **1.94×**。旧版本误把机柜级 4.4× 套用到单芯片（得出 5.7 PFLOPS），现已按官方页面更正。
...
| **HBM 总量** | **20.7 TB HBM3e**（AWS 官方） |
| **聚合内存带宽** | **706 TB/s**（AWS 官方） |
| **NeuronLink-v4** | NeuronSwitch-v1 全互联，**每芯片 2 TB/s**（AWS 官方） |
| **FP8 算力（机柜）** | **362 MXFP8 PFLOPS**（AWS 官方） |
...
| FP8 算力 | ~83 PFLOPS（64×1.3） | **~363 PFLOPS**（144×2.52） | **~4.4×** |
