...
**海光 DCU K100 AI 版（深算三号）** 是海光信息面向 AI 数据中心推出的**高性能 GPGPU 加速卡**，基于自研 **x86 兼容 GPGPU 架构**，**FP16/BF16 算力 192 TFLOPS**、**INT8 算力 392 TOPS**，配备 **64GB HBM2e 显存**和 **896 GB/s 显存带宽**。兼容 ROCm/DTK 软件栈，可**大幅降低 CUDA 迁移成本**，专为国产大模型训练与推理打造。（标准版 K100：FP64 24.5 TFLOPS、峰值约 100 TFLOPS、64GB HBM2e、896 GB/s、约 300W，保留双精度能力。）
...
| **INT8** | **392 TOPS** |
| **显存** | **64 GB HBM2e** |
| **显存带宽** | **896 GB/s**（HBM2e，双环形总线，实测利用率 92%+） |
| **总线拓扑** | 双环形 HBM2e 总线（读写分离，避免冲突） |
| **调度器** | 统一张量调度器，动态感知 Attention QKV 矩阵 |
| **TDP** | **350 W**（K100-AI 版；标准版 K100 约 300W） |
| **板卡形态** | PCIe 全高全长双宽卡 |
| **发布** | 2024 |
...
| **显存** | 64GB HBM2e | 96GB HBM3 | H20 1.5× |
...
|------|------|---------------|------|---------|------|
| **海光 DCU K100** | GPGPU/x86 | **192** | 64GB HBM2e | DTK (ROCm) | x86 兼容 |
