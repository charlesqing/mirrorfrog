...
## Hot Chips 2026 系统披露（2026-08 更新）

OpenAI 在 Hot Chips 2026 压轴系统披露 Jalapeño（演讲题目 *You Can Just Build Things … Chips*），这是其**首次公开自研推理 ASIC 的架构设计与性能测试结果**：

| 项目 | 披露内容 |
|------|----------|
| **TDP** | 约 **700 W** |
| **开发周期** | 从初始 RTL 到流片仅 **9 个月** |
| **设计方法** | AI 加速的硬件 + 软件协同设计（AI-accelerated HW/SW co-design） |
| **早期硅片表现** | 多芯片低延迟工作负载下达业界领先性能/瓦 |
| **定位** | 面向 OpenAI 推理负载的"最好的计算平台" |
| **形态** | ASIC + 宿主机 / 加速卡整机架对 |

> **关键洞察**：Jalapeño 与 Google TPU 8i、微软 Maia 200、Meta MTIA 400 在同一届会议上集体亮相，共同指向一个行业级迁移——**ASIC 正在规模足够大、模型结构稳定且 TCO 敏感的推理市场加速渗透**，而 GPU 则通过架构分化继续覆盖前沿训练与更广泛的通用负载。
