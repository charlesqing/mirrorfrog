...
**NVIDIA GB300 (Grace Blackwell 300)** 是 NVIDIA 第三代 **Grace Blackwell 超级芯片**（Superchip），**2025 H2 发布**。将 **2× B300 Ultra GPU**（升级版 Blackwell）通过 **NV-HBI 高速接口**与 **Arm Grace CPU** 配对，**单 Superchip = 2× B300 Ultra GPU + 1 Grace CPU**。36 颗 GB300 组成 **NVL72 机柜（共 72 颗 B300 Ultra + 36 颗 Grace）**，**NVLink 5 + ConnectX-9 1.6T 网络** 互联，是 **NVIDIA 数据中心 AI 的旗舰机柜级产品**。

**代际演进**：
- **GH200**（2023-Q3）：Grace Hopper，H100 + 72-core Arm Grace
- **GB200**（2024-Q4）：Grace Blackwell，2× B200 + 72-core Arm Grace
...
| **GPU 芯片** | **2× B300 Ultra**（Blackwell Ultra，每颗 288GB HBM3E） |
...
| **GPU 内存** | **576GB HBM3E**（2× B300 Ultra，每颗 288GB） |
| **GPU 带宽** | **20 TB/s**（2× B300 Ultra，每颗 10 TB/s） |
| **CPU 内存** | **480GB LPDDR5X** |
| **CPU 带宽** | **512 GB/s** |
| **FP4 sparse** | **30 PFLOPS**（2× B300 Ultra，单 GPU 15 PFLOPS） |
| **FP8 dense** | **7.5 PFLOPS**（2× B300 Ultra，单 GPU 3.75 PFLOPS） |
| **BF16 dense** | 3.75 PFLOPS（2× B300 Ultra） |
| **TDP（单 Superchip）** | **~3,000W**（2× B300 Ultra 1400W + Grace 200W） |
| **板卡形态** | 主板集成（不可拆） |
| **量产时间** | **2025 H2** |
...
| 项目 | 配置 |
|------|------|
| **Superchip 数** | **36× GB300**（每颗 2× B300 Ultra + 1 Grace） |
| **GPU 数** | 72× B300 Ultra |
...
| 互联 | NVLink 5 1.8 TB/s | UALink 1.3 TB/s | **GB300 1.4×** |
| 网络 | ConnectX-9 1.6T | Pensando 800G | **GB300 2×** |
| 软件 | CUDA | ROCm | NVIDIA 优势 |
| TDP | 1600W (Superchip) | 1000W | **MI400 -38%** |

> 注：MI400 是单 GPU vs GB300 Superchip（含 CPU）。纯 GPU 对比 MI400 优势明显（开放 HBM4 + 开放 UALink），但 NVL72 机柜 + ConnectX-9 仍是 NVIDIA 优势。
